...

{
    ...
    my $halfdec = do {
        ...
        if (substr($halfhex, 0, 2) ne '00' && substr($halfhex, -2) eq '00') { substr($halfhex, -4) = '1000'; }
    ...
    sub roundnear {
        ...
        my $rounded = $input;
        # rounding to 0, doesnt really make sense, but viewing it as a limit process it means do not round at all
        if ($targ != 0) {
            $rounded = ($input >= 0) ? $targ * int(($input + $halfdec * $targ) / $targ) : $targ * ceil(($input - $halfdec * $targ) / $targ);
        }
...
foreach my $test_case (@test_cases) {
    my $formula_name = 'Math::Business::BlackScholes::Binaries::' . ucfirst(lc($test_case->{type}));
    my %probs        = (
